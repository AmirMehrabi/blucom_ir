...
<div class="flex flex-wrap items-end justify-between gap-4">
    <div><h1 class="text-2xl font-extrabold text-[#071a3b]">خط‌های من</h1><p class="mt-2 text-slate-600">شماره‌ها، ارائه‌دهندگان و پاسخ‌گوها را یک‌جا ببینید.</p></div>
    <div class="flex flex-wrap gap-2"><a href="{{ route('customer.setup.provider') }}" class="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-700">افزودن ارائه‌دهنده</a><a href="{{ route('customer.setup.number') }}" class="rounded-xl bg-blue-600 px-4 py-3 text-sm font-bold text-white">افزودن شماره</a></div>
</div>
...
<section class="mt-5 grid gap-4 md:grid-cols-2">
    @forelse ($numbers as $number)
        <article class="panel p-5">
            ...
            <div class="mt-5 border-t border-slate-100 pt-4 text-sm text-slate-600">پاسخ‌گو: <strong class="text-slate-900">{{ $number->inboundRoute?->destination?->display_name ?: ($number->inboundRoute?->destination?->extension ?: 'هنوز انتخاب نشده') }}</strong></div>
            <div class="mt-4 flex gap-4 text-sm font-bold">@if ($number->status === 'disabled')<a class="text-blue-700" href="{{ route('customer.setup.numbers.edit', $number) }}">اصلاح درخواست</a>@else<a class="text-blue-700" href="{{ route('customer.setup.answer', $number) }}">{{ $number->inboundRoute ? 'تغییر پاسخ‌گو' : 'تعیین پاسخ‌گو' }}</a>@endif @if ($number->inboundRoute?->destination)<a class="text-blue-700" href="{{ route('customer.setup.phone', $number->inboundRoute->destination) }}">تنظیم تلفن</a>@endif</div>
        </article>
    @empty
        <div class="panel p-8 text-center md:col-span-2"><p class="font-bold">هنوز شماره‌ای ثبت نکرده‌اید.</p><p class="mt-2 text-sm text-slate-500">ابتدا ارائه‌دهنده خطتان را وصل کنید و سپس شماره خود را وارد کنید.</p><a class="mt-5 inline-block rounded-xl bg-blue-600 px-5 py-3 text-sm font-bold text-white" href="{{ route('customer.setup.provider') }}">شروع راه‌اندازی</a></div>
    @endforelse
</section>
