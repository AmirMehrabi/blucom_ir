...
For dialplan cutover, a live `mod_dialplan_xml` request has empty `tag_name`, `key_name`, and `key_value` fields and carries its context in `Hunt-Context`. A manual `xml_locate dialplan context name public` lookup uses the key fields instead, so it does not prove that live calls receive the same XML. Test both request shapes and inspect the actual call's transfer and bridge actions before retiring the static customer rules.
